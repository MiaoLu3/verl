...
#SBATCH --job-name=alfworld-qwen3-32b-val
...
echo "=============================================="
echo "ALFWorld Qwen3-32B val-only -- job starting"
...
export ALFWORLD_TRAJ_DUMP_DIR="${UPSTREAM_VERL}/trajectories/qwen3_32b_val_$(date +%Y%m%d_%H%M%S)"
...
bash "${ALFWORLD_RECIPE}/run_alfworld.sh" \
    ...
    trainer.experiment_name=agent_loop_qwen3_32b_val_only

EXIT_CODE=$?
echo "=============================================="
echo "ALFWorld Qwen3-32B val-only exited with code ${EXIT_CODE}"
echo "=============================================="
exit ${EXIT_CODE}

#!/bin/bash
# =============================================================================
# ALFWorld val-only on Qwen3-8B using upstream agent_loop framework.
# =============================================================================
# Re-uses submit_alfworld_agent_loop.sh's setup (cleanup, env, fail-fast,
# dump dir, etc) but overrides model path to Qwen3-8B and adds Qwen3-specific
...
#SBATCH --job-name=alfworld-qwen3-8b-rl-wt
...
echo "=============================================="
echo "ALFWorld Qwen3-8B val-only -- job starting"
...
export ALFWORLD_TRAJ_DUMP_DIR="${UPSTREAM_VERL}/trajectories/qwen3_8b_rl_wt_$(date +%Y%m%d_%H%M%S)"
...
# Run with Qwen3-8B override + enable_thinking=False (no_think mode) —
# matches verl-agent 284081's config which trained from 0.305 to 0.805 over
# 150 steps. With enable_thinking=False, the chat template pre-fills an
# empty <think></think> block so the model only needs to write
# <action>...</action>, which (combined with our relaxed projection that
# no longer requires <think> in raw_response) lets the policy be compact
# and stable. Previous thinking=True 4B run (285647) was oscillating
# val 0.26-0.54 and crashed with Ray socket error.
bash "${ALFWORLD_RECIPE}/run_alfworld.sh" \
    ...
    trainer.experiment_name=agent_loop_qwen3_8b_rl_with_think

EXIT_CODE=$?
echo "=============================================="
echo "ALFWorld Qwen3-8B val-only exited with code ${EXIT_CODE}"
echo "=============================================="
exit ${EXIT_CODE}
